...
const r2 = (n:number)=> Math.round(n*100)/100; // cents precision
...
interface P {
  id: string;
  name: string;
  stack: number;        // persists across hands
  inHand: boolean;      // false if folded
  streetCommit: number; // this street only
  totalCommit: number;  // whole hand
  hole: Card[];
  leaving: boolean;     // true if chose to leave after this hand
}
...
function blindsFor(dealer:number, n:number){
  if (n === 2){
    const sb = dealer;                 // BTN = SB heads-up
    const bb = 1 - dealer;
    return { sb, bb };
  }
  const sb = (dealer + 1) % n;
...
/** Build main + side pots from totalCommit. */
...
/** Compare two 5-card scores [Cat, k1..k5] — higher wins. */
function betterScore(a:number[], b:number[]){
  for (let i=0;i<6;i++){
...
/** Award odd chips (remainders) starting from first left of dealer among winners. */
function distributeOddCents(amount:number, winners:number[], dealerIdx:number, players:P[]){
  // amount is a small remainder like 0.01–(N*0.01)
  let cents = Math.round(amount*100); // turn into integer cents
  if (cents<=0 || winners.length===0) return;
  // Winner order starting left of dealer in table order
  const order: number[] = [];
  for (let i=0;i<winners.length;i++){
    const seat = (dealerIdx + 1 + i) % players.length;
...
/**
 * Real NLHE betting round:
 *  - currentBet = highest streetCommit
 *  - lastRaiseSize = size of last FULL raise; initial:
 *      - if currentBet==0 → BB (so first bet must be >= BB)
 *      - else → BB (so first RAISE must be >= BB over currentBet)
 *  - full-raise to = currentBet + max(BB, lastRaiseSize)
 *  - all-in < full raise does NOT reopen action
 *  - street ends when no one is awaiting action since the last full bet/raise
 *  - returns { ended, lastAgg } where lastAgg is index of last full bettor/raiser on this street (for showdown on river)
 */
async function bettingRound(
  ...
  BB:number,
  raiseCap:number
): Promise<{ended:boolean, lastAgg:number|null}> {
  const n = players.length;
  let currentBet = Math.max(...players.map(p=>p.streetCommit));
  let lastRaiseSize = BB;                       // seed per NLHE
  let raisesUsed = 0;                           // optional cap; 0 = unlimited
  const unlimited = raiseCap === 0;
  let lastAgg: number|null = null;

  // Who must act since the last full bet/raise?
  ...
  // Pre-seed awaiting for the start of the street:
  if (currentBet===0){
    addAwaitingAll();
  } else {
    for (const i of aliveIdxs){
      if (!isAllIn(players[i]) && players[i].streetCommit < currentBet) awaiting.add(i);
    }
  }
  ...
  while (safety-- > 0) {
    ...
    if (!players[seat].inHand || isAllIn(players[seat])) {
    ...
    const stillIn = players.filter(p=>p.inHand);
    if (stillIn.length===1) return { ended:true, lastAgg };

    const me = players[seat];
    const need = r2(currentBet - me.streetCommit);

    // If no one is awaiting action, the betting round is complete.
    if (awaiting.size === 0) break;
    ...
    if (need <= 0){
      ...
      if (me.stack > need){
        head = (canRaiseNow() && me.stack > need)
          ? (['fold','call','raise','allin'] as Action[])
          : (['fold','call','allin'] as Action[]);
      } else if (Math.abs(me.stack - need) < 1e-9){
        head = ['fold','call','allin'] as Action[]; // call equals all-in
      } else {
        head = ['fold','allin'] as Action[];        // short call allowed as all-in for less
      }
    }
    ...
    const doCheck = async ()=>{
      if (need > 0){
        console.log('Cannot check facing a bet.');
        ptr--;
        return false;
      }
    ...
    const doAllIn = async ()=>{
      const target = r2(me.streetCommit + me.stack); // shove everything
      ...
      if (wasRaise){
        if (raiseSize >= Math.max(BB, lastRaiseSize)-1e-9){
          ...
          raisesUsed++;
          lastAgg = seat;
          // full raise reopens action for all other non-all-in players
          const except = seat;
          awaiting.clear();
          for (const i of aliveIdxs) if (i!==except && !isAllIn(players[i])) awaiting.add(i);
          console.log(`${me.name} goes all-in to ${target} (FULL raise, size ${raiseSize}).`);
        } else {
          currentBet = target;  // not full → action not reopened
          awaiting.delete(seat);
    ...
    if (base==='leave'){ await doFold(); me.leaving = true; if (players.filter(p=>p.inHand).length===1) return {ended:true, lastAgg}; continue; }
    ...
    if (base==='bet'){
      ...
      post(me, add);
      // First bet defines lastRaiseSize = bet size (over 0)
      lastRaiseSize = to;
      currentBet = to;
      raisesUsed++;
      lastAgg = seat;
      // full bet reopens action for others
      const except = seat;
      awaiting.clear();
      for (const i of aliveIdxs) if (i!==except && !isAllIn(players[i])) awaiting.add(i);
    ...
    if (base==='raise'){
      ...
      lastRaiseSize = raiseSize;   // full raise resets lastRaiseSize
      currentBet = to;
      raisesUsed++;
      lastAgg = seat;
...
function showdownWithSidePots(board5: Card[], tablePlayers: P[], holes: Card[][], dealerIdx:number, riverAgg:number|null) {
  const survivorsIdx = tablePlayers.map((p, i)=> p.inHand ? i : -1).filter(i=>i>=0);
  if (survivorsIdx.length === 0){
    console.log('\nNo players at showdown.');
    return;
  }
  ...
  // Showdown order:
  //  - If river had aggression (riverAgg != null and that player is still in), they show first
  //  - Else first active player left of dealer
  let showStartIdx: number;
  if (riverAgg!==null && tablePlayers[riverAgg]?.inHand){
    showStartIdx = riverAgg;
  } else {
    // first active left of dealer
  ...
  const holeSurvivors = survivorsIdx.map(i=> holes[i]);
  const evalAll = evaluateHoldem(holeSurvivors, board5);

  console.log('\n--- SHOWDOWN ---');
  console.log('Showdown order: ' + showOrder.map(i=>tablePlayers[i].name).join(' → '));
  // Print in order (cosmetic)
  for (const seat of showOrder){
    const posInEval = survivorsIdx.indexOf(seat);
    const res = evalAll.results[posInEval];
    console.log(`${tablePlayers[seat].name}: ${res.bestFive.map(cardStr).join(' ')}  → ${scoreToString(res.score)}`);
  }

  // Pay each (side) pot to best of eligible; split ties fairly; odd cents go left of dealer
  pots.forEach((pot, k)=>{
    ...
    const each = r2(Math.floor(rawEach*100)/100); // even cents
...
async function postHandLobby(players:P[], dealerRef:{idx:number}, tableBuyIn:number){
  // Remove players who opted to leave during hand
  ...
  // Ask if anyone wants to leave now
  const leaveAns = (await ask('Anyone leaving the table? (type names separated by commas, or Enter for none): ')).trim();
  if (leaveAns){
    const names = leaveAns.split(',').map(s=>s.trim().toLowerCase()).filter(Boolean);
    table = table.filter(p => !names.includes(p.name.toLowerCase()));
  }

  // Rebuy for busted players
  for (const p of table){
    if (p.stack <= 0){
      const rb = (await ask(`Rebuy for ${p.name}? (y/N): `)).trim().toLowerCase();
      if (rb === 'y' || rb==='yes'){
        p.stack = tableBuyIn;
        console.log(`  -> ${p.name} rebuys for ${tableBuyIn}`);
      }
    }
  }

  // Add joiners
  let canAdd = Math.max(0, 6 - table.length);
  if (canAdd > 0){
    const j = (await ask(`How many joining? (0-${canAdd}): `)).trim();
    let nJoin = Number(j);
    if (!Number.isFinite(nJoin) || nJoin < 0) nJoin = 0;
    if (nJoin > canAdd) nJoin = canAdd;

    for (let k=0; k<nJoin; k++){
      const name = sanitizeName(table, (await ask(`  Name for new player ${k+1}: `)));
      const stack = tableBuyIn;
      table.push({
      ...
      console.log(`  -> ${name} joins with ${stack}`);
    }
  }

  if (table.length < 2){
    console.log('\nNot enough players to continue. Game ends.');
    return { table, continue:false };
  }

  // Keep dealer seat aligned to same *name* if still at table, else best-effort
  if (players.length > 0){
    const oldDealerName = players[dealerRef.idx]?.name;
    let newDealer = table.findIndex(p=>p.name === oldDealerName);
...
async function playOneHand(players:P[], dealer:number, SB:number, BB:number, ante:number, raiseCap:number){
  ...
  const deal = dealTexas(n);
  for (let i=0;i<n;i++) players[i].hole = deal.players[i];

  console.log('\n========================================');
  console.log(`New hand. Dealer: ${players[dealer].name}  (Antes + Blinds posted quietly)`);
  console.log('========================================');

  // Antes (from all players who have chips)
  if (ante > 0){
    for (const p of players) if (p.stack > 0) post(p, ante);
  }

  // Blinds
  post(players[sb], Math.min(SB, players[sb].stack));
  post(players[bb], Math.min(BB, players[bb].stack));
  if (ante > 0) console.log(`Antes: ${ante} from each`);
  ...
  resetStreet(players);
  console.log('\n--- FLOP ---');
  const boardFlop = deal.board.flop;
  console.log(cardsStr(boardFlop));
  ...
  console.log(cardsStr(boardTurn));
  br = await bettingRound(players, firstPost, BB, raiseCap);
  if (br.ended) { awardByFold(players); return players; }
  ...
  // Showdown (pass dealer and last river aggressor index)
  showdownWithSidePots(board5, players, deal.players, dealer, br.lastAgg);
  return players;
}
...
async function main() {
  ...
  const raiseCap = Number(capStr) >= 0 ? Number(capStr) : 0; // default unlimited
  ...
  console.log('\nTexas Hold’em — Table Mode (2–6 players)');
